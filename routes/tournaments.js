...
router.post('/', auth, creatorAuth, [
  body('name')
    ...
    .trim()
    .isLength({ max: 100 })
    .withMessage('Tournament name is required and cannot exceed 100 characters'),
  body('game')
    .isIn(['League of Legends', 'Valorant', 'CS2', 'Fortnite', 'Overwatch 2', 'Dota 2', 'TFT'])
    .withMessage('Invalid game selection'),
  body('maxParticipants')
    .isInt({ min: 2, max: 256 })
    .withMessage('Max participants must be between 2 and 256'),
  body('entryFee')
    .isFloat({ min: 0 })
...
router.post('/:id/join', auth, async (req, res) => {
  try {
    ...
    if (tournament.teamSize > 1) {
      ...
      const existingTeamParticipant = tournament.participants.find(p => 
        p.team && p.team.toString() === teamId
      );
...
router.post('/:id/start', auth, async (req, res) => {
  try {
    ...
    tournament.bracket = generateBracket(tournament.participants, tournament.format);
    await tournament.save();
...
function generateBracket(participants, format) {
  ...
  while (currentRound.length > 1) {
    ...
    for (let i = 0; i < currentRound.length; i += 2) {
      if (i + 1 < currentRound.length) {
        matches.push({
          ...
          participant1: currentRound[i]._id || currentRound[i].user || currentRound[i].team,
          participant2: currentRound[i + 1]._id || currentRound[i + 1].user || currentRound[i + 1].team,
          status: 'pending'
        });
      } else {
        ...
        matches.push({
          matchId: `R${roundNumber}M${Math.floor(i/2) + 1}`,
          participant1: currentRound[i]._id || currentRound[i].user || currentRound[i].team,
          winner: currentRound[i]._id || currentRound[i].user || currentRound[i].team,
          status: 'completed'
        });
      }
    }
    ...
    currentRound = matches.filter(m => m.status === 'completed').map(m => ({ _id: m.winner }));
    currentRound = currentRound.concat(matches.filter(m => m.status === 'pending'));
    roundNumber++;
  }
