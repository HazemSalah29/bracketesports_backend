...
router.post('/apply', auth, [
  ...
  try {
    ...
    const validGames = ['League of Legends', 'Valorant', 'CS2', 'Fortnite', 'Overwatch 2', 'Dota 2', 'TFT'];
    const invalidSpecialties = specialties.filter(game => !validGames.includes(game));
...
router.put('/profile', auth, creatorAuth, [
  body('displayName')
    .optional()
    .trim()
    .isLength({ max: 50 })
...
router.post('/:id/rate', auth, [
  ...
  try {
    ...
    const existingRating = creatorProfile.ratings.find(r => 
