...
router.post('/link-riot', auth, [
  ...
  body('gameTag')
    .notEmpty()
    .trim()
    .withMessage('Game tag is required'),
  body('game')
    .isIn(['League of Legends', 'Valorant', 'TFT'])
    .withMessage('Invalid game selection'),
  body('region')
    .isIn(['NA1', 'EUW1', 'EUN1', 'KR', 'JP1', 'BR1', 'LAN', 'LAS', 'OCE1', 'TR1', 'RU'])
  ...
  try {
    ...
    try {
      // Get account by Riot ID (username#gameTag)
      const regionalUrl = getRegionalUrl(region);
      const accountResponse = await axios.get(
        `${regionalUrl}/riot/account/v1/accounts/by-riot-id/${encodeURIComponent(username)}/${encodeURIComponent(gameTag)}`,
        {
          headers: {
      ...
      const gamingAccount = new GamingAccount({
        ...
        stats: {
          ...
          winRate: gameSpecificData.wins + gameSpecificData.losses > 0 
            ? (gameSpecificData.wins / (gameSpecificData.wins + gameSpecificData.losses) * 100).toFixed(1)
            : 0,
      ...
      console.error('Riot API error:', apiError.response?.data || apiError.message);
      
      if (apiError.response?.status === 404) {
        return res.status(404).json({
          success: false,
          message: 'Riot account not found. Please check your username and tag.'
        });
      }
  ...
  try {
    ...
    if (gamingAccount.platform === 'riot') {
      try {
        ...
        if (gamingAccount.game === 'League of Legends' && gamingAccount.summonerId) {
          ...
          gamingAccount.stats = {
            ...
            wins: soloQueueData ? soloQueueData.wins : 0,
            losses: soloQueueData ? soloQueueData.losses : 0,
            winRate: soloQueueData && (soloQueueData.wins + soloQueueData.losses) > 0 
              ? (soloQueueData.wins / (soloQueueData.wins + soloQueueData.losses) * 100).toFixed(1)
              : 0,
            totalGames: soloQueueData ? soloQueueData.wins + soloQueueData.losses : 0
          };
        }
        ...
        console.error('API refresh error:', apiError.response?.data || apiError.message);
        res.status(500).json({
          success: false,
