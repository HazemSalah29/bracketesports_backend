...
const gamingAccountSchema = new mongoose.Schema({
  ...
  game: {
    ...
    enum: ['League of Legends', 'Valorant', 'TFT', 'CS2', 'Dota 2', 'Fortnite', 'Overwatch 2']
  },
  username: {
    type: String,
  ...
  region: {
    ...
    enum: ['NA1', 'EUW1', 'EUN1', 'KR', 'JP1', 'BR1', 'LAN', 'LAS', 'OCE1', 'TR1', 'RU']
  },
  accountId: {
    type: String,
...
gamingAccountSchema.virtual('rankString').get(function() {
  if (this.stats.currentRank && this.stats.currentRank.tier) {
    return `${this.stats.currentRank.tier} ${this.stats.currentRank.division || ''} ${this.stats.currentRank.lp || 0} LP`;
  }
  return 'Unranked';
});
